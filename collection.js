(() => {
  ...
  const CANONICAL_PRODUCTS = [
    ...
    },
    {
      ...
      price: 7999,
      oldPrice: null,
      img: "assets/images/product_phantom.png",
      stars: 5,
      reviews: 96,
      new: false,
      bestseller: true,
    ...
    },
    {
      ...
      price: 7499,
      oldPrice: 8299,
      img: "assets/images/product_boost.png",
      stars: 5,
      reviews: 142,
      new: false,
      bestseller: false,
    ...
    },
    {
      ...
      price: 4999,
      oldPrice: null,
      img: "assets/images/product_lite.png",
      stars: 5,
      reviews: 64,
      new: false,
      bestseller: false,
    ...
    },
    {
      ...
      price: 6799,
      oldPrice: 7999,
      img: "assets/images/product_trail.png",
      stars: 5,
      reviews: 41,
      new: false,
      bestseller: false,
      discount: 15,
      color: "white",
      features: ["grip", "waterproof"],
      status: "Active",
      stock: 0,
      featured: false,
      sku: "NOVA-TR-008",
      desc: "Men's Hiking Shoes",
    },
    {
      id: "nova-motion",
      name: "Nova Motion",
    ...
    },
    {
      ...
      price: 10999,
      oldPrice: null,
      img: "assets/images/product_carbon.png",
      stars: 5,
      reviews: 19,
      new: false,
      bestseller: false,
    ...
    },
    {
      ...
      price: 4799,
      oldPrice: 5499,
      img: "assets/images/product_flex.png",
      stars: 5,
      reviews: 22,
      new: false,
      bestseller: false,
  ...
  function toCollectionProduct(ap) {
    const disc =
      ap.oldPrice && ap.price
        ? Math.round((1 - ap.price / ap.oldPrice) * 100)
        : ap.discount || 0;
    return {
      id: ap.id,
      name: ap.name,
  ...
  function parseQueryParams() {
    ...
    if (category) {
      ...
      const checkbox = filtersForm.querySelector(
  ...
  function renderProducts(products) {
    ...
    if (products.length === 0) {
      ...
      `;
      document
        .getElementById("resetFiltersLink")
    ...
    products.forEach((prod) => {
      ...
      if (prod.bestseller)
        badgesHtml += `<span class="badge badge--best">Bestseller</span>`;
      if (prod.discount > 0)
        badgesHtml += `<span class="badge badge--sale">-${prod.discount}%</span>`;
  ...
  function filterAndSortProducts() {
    let filtered = PRODUCTS.filter((prod) => {
      ...
      if (
        ...
        !activeFilters.category.includes(prod.category)
      )
        return false;
      ...
      if (
        ...
        !activeFilters.gender.includes(prod.gender)
      )
        return false;

      // Size filter
      if (activeFilters.size.length > 0) {
        const matchesSize = prod.size.some((s) =>
          activeFilters.size.includes(s.toString()),
        );
      ...
      if (
        ...
        !activeFilters.color.includes(prod.color)
      )
        return false;

      // Features filter
      if (activeFilters.feature.length > 0) {
        const matchesAllFeatures = activeFilters.feature.every((f) =>
      ...
      if (activeFilters.tag.length > 0) {
        ...
        if (activeFilters.tag.includes("bestseller") && !prod.bestseller)
          return false;
      }
    ...
    cartItemsList.addEventListener("click", async (e) => {
      ...
      if (dec) {
        const item = cart.find(
          (i) => i.id === dec.dataset.id && i.size === Number(dec.dataset.size),
        );
        await cartState.updateQty(
          dec.dataset.id,
          Number(dec.dataset.size),
          item ? item.itemId : null,
          -1,
        );
      } else if (inc) {
        const item = cart.find(
          (i) => i.id === inc.dataset.id && i.size === Number(inc.dataset.size),
        );
        await cartState.updateQty(
          inc.dataset.id,
          Number(inc.dataset.size),
          item ? item.itemId : null,
          1,
        );
      } else if (remove) {
        const item = cart.find(
          (i) =>
            i.id === remove.dataset.id &&
            i.size === Number(remove.dataset.size),
        );
        await cartState.remove(
          remove.dataset.id,
          Number(remove.dataset.size),
          item ? item.itemId : null,
        );
      }
    });
  ...
  function openQuickView(prodId) {
    ...
    modalAddBtn.addEventListener("click", async (e) => {
      ...
      const selectedSize = Number(
        modalContent.querySelector('input[name="modal-size"]:checked').value,
      );
      await cartState.add(
        prod.id,
        prod.name,
  ...
  function updateFilterState() {
    ...
    // Genders
    activeFilters.gender = Array.from(
      filtersForm.querySelectorAll('input[name="gender"]:checked'),
    ).map((el) => el.value);

    // Sizes
    activeFilters.size = Array.from(
      filtersForm.querySelectorAll('input[name="size"]:checked'),
    ).map((el) => el.value);

    // Color swatches
    activeFilters.color = Array.from(
      filtersForm.querySelectorAll('input[name="color"]:checked'),
    ).map((el) => el.value);

    // Features
    activeFilters.feature = Array.from(
      filtersForm.querySelectorAll('input[name="feature"]:checked'),
    ).map((el) => el.value);
  }
